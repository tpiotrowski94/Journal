      ...
      <div className="bg-[#1e293b] rounded-3xl border border-slate-700 overflow-hidden shadow-2xl flex flex-col">
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left table-auto">
            <thead>
              <tr className="bg-slate-900/40 text-slate-500 text-xs font-black uppercase tracking-widest border-b border-slate-700">
                  ...
                  <React.Fragment key={trade.id}>
                    <tr className={`${idx % 2 === 0 ? 'bg-transparent' : 'bg-slate-800/20'} hover:bg-slate-700/30 transition-colors group border-b border-slate-800/50`}>
                      <td className="px-6 py-5 align-top">
                        <div className="font-black text-white text-lg tracking-tight group-hover:text-blue-400 transition-colors flex items-center gap-2">
                          {trade.symbol}
                          <div className={`text-[10px] font-black px-1.5 py-0.5 rounded-sm uppercase tracking-tight ${trade.type === TradeType.LONG ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-rose-500/20 text-rose-400 border border-rose-500/30'}`}>
                            {trade.type}
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5 mt-1.5">
                          <span className="text-xs text-blue-500 font-black uppercase">{trade.leverage}x</span>
                          <span className="bg-slate-900 px-1.5 py-0.5 rounded text-[10px] font-black text-slate-500 uppercase border border-slate-700">{trade.marginMode}</span>
                        </div>
                        <div className="mt-3 flex items-center gap-2 group/confidence">
                          <div className="flex gap-0.5">
                            {[1, 2, 3, 4, 5].map(i => (
                              <button
                                key={i}
                                onClick={() => onEditTrade(trade.id, { confidence: i })}
                                className={`w-2 h-1.5 rounded-full transition-all ${i <= conf
                                  ? (conf <= 2 ? 'bg-rose-500' : conf === 3 ? 'bg-amber-500' : 'bg-emerald-500')
                                  : 'bg-slate-700 hover:bg-slate-500'
                                  }`}
                                title={`Rate: ${confidenceLabels[i - 1]}`}
                              />
                            ))}
                          </div>
                          <span className="text-[10px] font-black text-slate-500 uppercase tracking-tighter group-hover/confidence:text-slate-300 transition-colors cursor-default">
                      ...
                      <td className="px-6 py-5 align-top">
                        <div className="flex flex-col">
                          <div className="text-white font-black text-base tracking-tighter">
                          ...
                          <div className="text-xs text-slate-400 font-bold mt-0.5 flex items-center gap-1">
                            <i className="fas fa-coins text-[10px] text-slate-600"></i>
                            {amountVal.toLocaleString(undefined, { maximumFractionDigits: 8 })} <span className="text-[10px] text-slate-500">{trade.symbol.split('/')[0]}</span>
                          </div>
                          <div className="mt-2 flex items-center gap-1.5">
                            <span className="text-[10px] font-black text-slate-600 uppercase">Est. Margin:</span>
                            <span className="text-xs font-black text-emerald-500/80">${marginVal.toFixed(2)}</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-5 align-top">
                        <div className="space-y-1.5">
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] w-14 text-slate-600 font-black uppercase">Avg Entry</span>
                            <span className="font-mono text-sm text-slate-200 font-bold">${entryVal.toLocaleString()}</span>
                          </div>
                          {!isOpenTable && (
                            <>
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] w-14 text-blue-500 font-black uppercase">Exit Price</span>
                                <span className="font-mono text-sm text-blue-400 font-bold">${Number(trade.exitPrice).toLocaleString()}</span>
                              </div>
                              {exitFeesVal > 0 || exitFundingVal !== 0 ? (
                                <div className="text-[10px] font-black uppercase text-amber-500/70 italic mt-1 bg-amber-500/5 px-2 py-1 rounded border border-amber-500/10 w-fit">
                                  Incremental Exit: ${(exitFeesVal + exitFundingVal).toFixed(2)}
                                </div>
                              ) : (
                                <div className="text-[10px] font-black uppercase text-slate-600 italic mt-1 px-2 py-1 rounded border border-slate-700/50 w-fit">
                                  Total Settle Costs: ${totalCosts.toFixed(2)}
                                </div>
                              )}
                            </>
                          )}
                          <div className="text-[10px] text-slate-500 font-bold uppercase mt-2 opacity-60 flex flex-col">
                            <span className="text-[10px] tracking-widest text-slate-600">Entry: {formatTradeDate(trade.date)}</span>
                            {!isOpenTable && <span className="text-[10px] tracking-widest text-blue-500/60 mt-0.5">Exit: {formatTradeDate(trade.exitDate)}</span>}
                          </div>
                        </div>
                      </td>

                      <td className="px-6 py-5 align-top">
                        <div className={`text-base font-black ${totalCosts > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                          {totalCosts > 0 ? '-' : '+'}${Math.abs(totalCosts).toFixed(2)}
                        </div>
                        <div className="mt-1 space-y-0.5">
                          <div className="text-[10px] text-slate-500 uppercase font-black opacity-60">Trading: ${feesVal.toFixed(2)}</div>
                          <div className={`text-[10px] uppercase font-black flex items-center gap-1 ${fundingVal > 0 ? 'text-amber-500' : fundingVal < 0 ? 'text-emerald-500' : 'text-slate-500'}`}>
                      ...
                      <td className="px-6 py-5 text-right align-top">
                        {isOpenTable ? (
                          ...
                          <div className="leading-none flex flex-col items-end gap-0.5">
                            {/* 1. Net ROI % (Main Percentage) */}
                            <div className={`text-2xl font-black tracking-tight mb-1 ${netRoe >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                              {netRoe >= 0 ? '+' : ''}{netRoe.toFixed(2)}%
                            </div>

                            {/* 2. Dollars from Position (Gross PnL) */}
                            <div className="flex items-center gap-1.5 opacity-60">
                              <span className="text-[9px] font-black uppercase text-slate-500">Gross:</span>
                              <span className={`text-[10px] font-bold ${grossPnl >= 0 ? 'text-slate-300' : 'text-rose-300'}`}>
                                {grossPnl >= 0 ? '+' : ''}{grossPnl.toFixed(2)} $
                              </span>
                            </div>

                            {/* 3. Dollars after Costs (Net PnL) */}
                            <div className="flex items-center gap-1.5">
                              <span className="text-[9px] font-black uppercase text-slate-500">Net:</span>
                              <span className={`text-base font-black ${pnlVal >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                                {pnlVal >= 0 ? '+' : ''}{pnlVal.toFixed(2)} $
                              </span>
                            </div>
                          </div>
                          ...
                          {trade.status === TradeStatus.OPEN ? (
                            <>
                              <button
                                ...
                                className="bg-blue-600 text-white hover:bg-blue-500 px-3 py-1.5 rounded-xl text-xs font-black uppercase transition-all shadow-lg"
                              >
                                Exit
                              </button>
                              <button
                                onClick={() => setModalState({ type: 'ADD', trade })}
                                className="bg-slate-700 text-slate-300 hover:text-white px-3 py-1.5 rounded-xl text-xs font-black uppercase transition-all border border-slate-600"
                              >
                                Add
                              </button>
                            </>
                        ...
                        <div className="ml-6 space-y-2 relative border-l-2 border-slate-700/50 pl-6 py-1">
                          {sortedNotes.length > 0 ? sortedNotes.map((note) => (
                            <div key={note.id} className="relative group/note flex flex-col gap-0.5 mb-2">
                              ...
                              <div className="flex items-center gap-3">
                                <span className="text-xs font-black text-slate-600 uppercase tracking-tighter">{formatTradeDate(note.date)}</span>
                                <div className="flex gap-3 opacity-0 group-hover/note:opacity-100 transition-opacity">
                                  <button onClick={() => setModalState({ type: 'EDIT_NOTE', trade, extra: { note } })} className="text-[10px] text-blue-500 font-black uppercase">Edit</button>
                                  <button onClick={() => onDeleteNote(trade.id, note.id)} className="text-[10px] text-rose-500 font-black uppercase">Delete</button>
                                </div>
                              </div>
                              <p className="text-xs text-slate-400 font-medium leading-relaxed max-w-3xl whitespace-pre-wrap">{note.text}</p>
                            </div>
                          )) : (
            ...
            <div className="flex gap-1">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-xs font-black uppercase transition-all ${currentPage === page
